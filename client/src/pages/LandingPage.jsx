...
// Reusable city-selection button
...
export default function LandingPage({ user, setUser }) {
  ...
  // Debounced fetch
  const debouncedFetchListings = debounce(async (city, budget, rating, places) => {
    try {
      setError(null);
      ...
      url.searchParams.set("places", JSON.stringify(places));
      ...
      console.log(`Response status: ${response.status}, data: ${JSON.stringify(response.data).slice(0, 200)}...`);
      setListings(response.data);
    } catch (err) {
      const errorMessage = err.response
        ? `API error: ${err.response.status} - ${err.response.data?.error || err.response.data?.details || 'Unknown error'}`
        : `Network error: ${err.message}. Ensure backend is running at ${API_URL} and CORS is enabled. Check client/.env (REACT_APP_API_URL or VITE_API_URL).`;
  ...
  const handleCitySelect = (cityCode) => {
    const realCity = cityMap[cityCode] || cityCode;
    setSelectedCity(realCity);
    setTimeout(() => listingsRef.current?.scrollIntoView({ behavior: "smooth" }), 300);
  };
  

  // Fetch listings on city or filter change
  useEffect(() => {
    if (!selectedCity) {
      setListings([]);
      setError(null);
    ...
    debouncedFetchListings(selectedCity, budgetRange, ratingRange, places);
  ...
  Hello, {userData?.firstName || user.email}
    ...
    >
      ...
      <section className="relative w-full h-screen">
        ...
        >
          <h1 className="text-3xl font-bold mb-2 text-[#545E7B]">
            Select Your City
          </h1>
          <p className="text-base text-[#131D35]">
  ...
  Explore Housing Options in {selectedCity 
    ...
    : "Your City"}
          ...
          <div className="w-full md:w-1/3 mb-6 md:mb-0">
            ...
            <Range
              ...
              renderTrack={({ props, children }) => {
                ...
                const left = ((min - 500) / (4000 - 500)) * 100;
                const width = ((max - min) / (4000 - 500)) * 100;
                return (
                  <div
                    {...props}
                    ref={props.ref}
        ...
        {listings.length > 0 ? (
          ...
          <p className="text-center text-gray-600">
            ...
            Debug: Ensure backend is running at {API_URL}/listings/search and CORS allows requests from frontend.
          </p>
        )}
      </section>
    </motion.div>
